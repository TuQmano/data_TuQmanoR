...
discurso_completo <- "Señora Vicepresidenta, Diputadas, Diputados y Senadores
...
Estos muros, y no nuestras ideas distintas, son los que nos dividen en este tiempo histórico. Por eso quisiera que estas palabras no fueran un monólogo, sino la invitación a una reflexión profunda y sincera acerca de este momento trascendental.
...
El nivel de producción industrial hoy es equivalente al del año 2006: retrocedimos 13 años. El empleo industrial registrado tiene el nivel de 2009.
...
23 de las 24 ramas de la industria bajaron su nivel de actividad en 2018 respecto de 2015. En estos 4 años se perdieron en la industria más de 141 mil empleos registrados del sector privado.
...
La consistencia integral de lo que proponemos en materia de todas las variables del plan -precios, salarios, tarifas, tipo de cambio, aspectos monetarios, fiscales y sociales-, serán explicitadas en los próximos días, convocando a todos los sectores involucrados. Apelo a la responsabilidad y el patriotismo de todas y todos.
...
Existe otro equilibrio básico que tenemos que construir: el equilibrio federal y territorial. Argentina necesita poner fin a una estructura que muestra un país “central” rico y pujante y un país “periférico” que busca desarrollarse a partir de las mínimas concesiones que el país central entrega. No pueden haber argentinos de primera y argentinos de segunda.

Argentina es una sola y mancomunadamente debe propender al desarrollos de todas y cada una de sus regiones. Ese es el desafío que enfrentamos y debemos superar.
...
Así como ahora el Instituto Nacional de Vitivinicultura funciona en la Provincia de Mendoza y el Instituto Nacional de Investigación y Desarrollo Pesquero funciona en la Ciudad de Mar del Plata, debemos pensar en diversas alternativas que garanticen un nuevo federalismo. Vamos a poner a la Argentina de pie, con una infraestructura federal de calidad, sostenible y sustentable, promoviendo el desarrollo regional y creando juntos miles de puestos de trabajo en cooperativas de servicio, pequeñas y grandes empresas.
...
Una democracia sin justicia realmente independiente no es democracia. Supo decir un penalista clásico, que cuando la política ingresa a los Tribunales, la justicia escapa por la ventana.
...
Porque una justicia demorada y manipulada significa una democracia acosada y denegada. Queremos una Argentina donde se respeten a rajatabla la Constitución y las leyes.
...
No habrá pauta del Estado para financiar programas individuales de periodistas. Sólo se destinará a instituciones periodísticas. En la relación con los periodistas, más que nunca tiene sentido aquella frase de que “las cuentas claras conservan la amistad y el respeto”. En el mismo contexto de innovación, vamos a proponer una Gran Escuela de Gobierno, con altísima excelencia académica, como eje de un proceso de profesionalización, mérito y carrera administrativa en el marco del Estado Nacional.
...
Permítanme también recordar en esta hora a tres personas que me signaron en la vida. Quiero recordar a mi madre que me marcó con su ejemplo.
...
Quisiera que dejemos como huella haber reconstruido la casa común con un gran proyecto nacional, un Acuerdo Estratégico para el Desarrollo, del cual nos sintamos orgullosos. Por eso, deseo que las palabras finales de mi primer mensaje como Presidente de toda la Argentina, no constituyan una respuesta sino una pregunta.
...
#transformo discurso tomando cada palabra como un valor de una fila 
palabras <- tokenize_words(discurso_completo) %>%  # CREA UNA LISTA
  print()


#Creo tibble desde lista
palabras_df <- palabras %>% 
  unlist() %>% 
  as_tibble()  %>% print()
...
nube_df <- palabras_df  %>%  # 7218 palabras
  filter(!value %in% stopwords::stopwords(language = "es")) %>%  # 3646 palabras sin stopwords 
  group_by(value) %>%  # 1954 palabras unicas
  mutate(n = n()) %>%  # cuento frecuencia de repeticion
  arrange(desc(n)) %>% 
  distinct() %>% print() #dejo unicas
